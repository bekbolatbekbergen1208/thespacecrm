...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const effectiveRoutes = effectiveEmployeeRoutes({ allowedRoutes, dashboardRoute, position: membership.position, businessType: company?.business_type ?? null });
  const employeeHomeRoute = defaultEmployeeHomeRoute({ allowedRoutes, dashboardRoute, position: membership.position, businessType: company?.business_type ?? null });
